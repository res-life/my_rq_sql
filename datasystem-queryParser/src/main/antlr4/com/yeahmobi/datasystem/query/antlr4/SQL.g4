...
@parser::members {
    ...
    public boolean isJoinSql = false;
    public String dataSourceName2 = null;
...
query
    : select_stmt join_stmt where_stmt (groupby_stmt)?
    ;

select_stmt
    : 'select' e+=aliasedExpression (',' e+=aliasedExpression)* 'from' datasource datasource_alias? {
    ...
    ;
join_stmt
	: ('join' | 'inner' 'join' | 'left' 'join' | 'right' 'join' | 'full' 'join') IDENT IDENT? 'on' IDENT2 '=' IDENT2 ('and' IDENT2 '=' IDENT2)*{
	  isJoinSql = true;
	}
	;
...
aggregate returns [AggregatorFactory agg]
    : fn=( SUM^ | MIN^ | MAX^ ) OPEN! name=(IDENT2|COUNT) CLOSE! { $agg = evalAgg($name.text, $fn.type); }
