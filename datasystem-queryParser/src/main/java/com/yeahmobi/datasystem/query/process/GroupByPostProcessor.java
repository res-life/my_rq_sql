...
public class GroupByPostProcessor extends PostProcessor {

	public GroupByPostProcessor(DruidReportParser parser) {
		super(parser);
	}
	...
	public ReportResult process(List<?> input) {
		...
		for (final Row r : rows) {
			reportResult.append(Iterables.toArray(
					Iterables.concat(Iterables.transform(parser.intervalUnits, new TimeStampTransformer(r.getTimestampFromEpoch(), parser.timeZone)),
							Iterables.transform(timeDimensions.values(), new DimensionTransformer(r, dimensionTable)),
		...
		parser.intervalUnits.addAll(parser.isDoTimeDb? timeDimensions.keySet() : Collections.EMPTY_LIST);
