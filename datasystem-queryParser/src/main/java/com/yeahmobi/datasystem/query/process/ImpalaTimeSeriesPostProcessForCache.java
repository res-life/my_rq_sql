...
public class ImpalaTimeSeriesPostProcessForCache extends PostProcessor {

    private static Logger logger = Logger.getLogger(ImpalaTimeSeriesPostProcessForCache.class);

    public ImpalaTimeSeriesPostProcessForCache(DruidReportParser parser) {
        super(parser);
    }
    ...
    public ReportResult process(List<?> input) {
        ...
        for(Result<TimeseriesResultValue> r : rows) {
                reportResult.append(
                        Iterables.toArray(
                                Iterables.concat(
                                        Iterables.transform(timeDoDb.values(), new TimeSeriesMetricDimensionTranIgnoreTime(r, dimensionTable)),
