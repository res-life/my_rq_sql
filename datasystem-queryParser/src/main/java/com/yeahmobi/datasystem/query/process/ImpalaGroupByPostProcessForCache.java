...
public class ImpalaGroupByPostProcessForCache extends PostProcessor {

	public ImpalaGroupByPostProcessForCache(DruidReportParser parser) {
		super(parser);
	}
	...
	public ReportResult process(List<?> input) {
		...
		for (final Row r : rows) {
			reportResult.append(Iterables.toArray(
					Iterables.concat(Iterables.transform(timeDimensions.values(), new DimensionTransformerIgnoreTime(r, dimensionTable)),
							Iterables.transform(parser.groupByDimensions.values(), new DimensionTransformerIgnoreTime(r, dimensionTable)),
